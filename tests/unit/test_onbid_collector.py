...
class TestOnbidCollectorInit:
    """Tests for OnbidCollector initialization."""

    def test_init_with_api_key_from_config(self):
        """Test initialization with API key in config."""
        source = {
            "api_key": "test_key",
            "base_url": "https://api.go.kr/B010003",
        }

        collector = OnbidCollector("onbid_test", source)
    ...
    def test_init_with_api_key_from_env(self):
        ...
        source = {"base_url": "https://api.go.kr/B010003"}
    ...
    def test_init_without_api_key_raises_error(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            with pytest.raises(ValueError, match="requires 'api_key'"):
                OnbidCollector("onbid_test", source)
...
class TestOnbidCollectorCollect:
    """Tests for OnbidCollector.collect() method."""

    @pytest.fixture
    def collector(self):
        """Create OnbidCollector instance."""
        source = {
            "api_key": "test_key",
            "base_url": "https://api.go.kr/B010003",
        }
        return OnbidCollector("onbid_test", source)

    @pytest.fixture
    def sample_json_response(self):
        """Sample JSON response from Onbid API."""
        return {
            "response": {
                "body": {
                    "items": [
    ...
    def test_collect_successful_request(self, collector, sample_json_response):
        ...
        with patch("requests.get") as mock_get:
            ...
            assert len(items) == 2
            assert items[0].title.startswith("서울 강남구 아파트")
            assert items[1].title.startswith("경기 성남시 빌라")
    ...
    def test_collect_parses_prices_correctly(self, collector, sample_json_response):
        ...
        with patch("requests.get") as mock_get:
            ...
            assert items[1].price == 280000000
...
class TestOnbidCollectorParseItem:
    """Tests for OnbidCollector._parse_item() method."""
    ...
    def test_parse_item_with_complete_data(self, collector):
        ...
        assert raw_item.title == "서울 강남구 아파트 - 서울 강남구"
        assert raw_item.price == 480000000
        assert raw_item.area == 84.5
        assert raw_item.property_type == "아파트"
        assert raw_item.region == "서울 강남구"
    ...
    def test_parse_item_uses_winning_bid_price(self, collector):
        """Test that winning bid price is preferred."""
        item_data = {
            "cltrNo": "2024001",
            "cltrNm": "서울 강남구 아파트",
            "appraisalPrice": "500000000",
    ...
    def test_parse_item_falls_back_to_appraisal_price(self, collector):
        """Test fallback to appraisal price when winning bid not available."""
        item_data = {
            "cltrNo": "2024001",
            "cltrNm": "서울 강남구 아파트",
            "appraisalPrice": "500000000",
    ...
    def test_parse_item_builds_correct_url(self, collector):
        ...
        assert raw_item.url == "onbid://auction/2024001"
    ...
    def test_parse_item_raw_data_completeness(self, collector):
        ...
        assert "cltrNo" in raw_item.raw_data
        assert "cltrNm" in raw_item.raw_data
        assert "appraisalPrice" in raw_item.raw_data
        assert "winningBidPrice" in raw_item.raw_data


class TestOnbidCollectorDateParsing:
    """Tests for date parsing in OnbidCollector."""
...
class TestOnbidCollectorAreaParsing:
    """Tests for area parsing in OnbidCollector."""
