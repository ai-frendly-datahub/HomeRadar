...
# 1회 수집 실행 (날짜별 리포트와 DB 스냅샷 생성 포함)
python main.py --mode once --generate-report --snapshot-db

# 정기 수집 스케줄러 실행 (24시간 간격, 날짜별 기록 포함)
...
- `MOLIT_SERVICE_KEY`: 국토부 실거래가/전월세 API 키.
- `SUBSCRIPTION_API_KEY`: 청약 공고/경쟁률 API 키.
- `ONBID_API_KEY`: 온비드 공매 API 키.
- `config/config.yaml`: 기본 graph DB, raw log, report, search index 경로를 정의합니다.
- 날짜별 기록:
  - raw JSONL: `data/raw/YYYY-MM-DD/<source>.jsonl`
  - DuckDB snapshot: `data/snapshots/YYYY-MM-DD/homeradar.duckdb` (`--snapshot-db`)
  - HTML report: `reports/daily_report_YYYYMMDD.html`, 최신 사본 `reports/daily_report.html`

## 소스 활성화 정책

- 기본 활성화 공식 소스: MOLIT 매매, MOLIT 전월세, REB 청약, Onbid 공매
- 기본 활성화 시장 소스: 주요 부동산 뉴스 RSS
- 비밀값이 없는 공식 API는 실패로 종료하지 않고 해당 소스만 스킵합니다.
- `Naver Land`, `Zigbang`, `browser` 계열은 운영 안정성이 더 필요해 아직 기본 비활성 상태입니다.
- 데이터 품질 기준은 `docs/data-quality-plan.md`와 `config/sources.yaml`의 `data_quality` 섹션을 기준으로 관리합니다.
- 공식 운영 소스는 freshness SLA와 canonical key를 명시하고, 민간 listing 후보는 `source_backlog`에서 ToS/anti-bot/stale listing 검증 후 활성화합니다.
- 검증 상태는 `official_primary`, `official_policy_corroboration`, `market_corroboration_requires_official_source`, `backlog_candidate`로 분리합니다.
- 수집 item에는 `raw_data.home_quality`와 `HomeVerificationState`를 추가해 뉴스/정책 맥락이 공식 거래·청약·공매 fact를 덮어쓰지 않도록 합니다.
- 저장 후에는 `urls.verification_state`, `verification_role`, `event_model`, `merge_policy`로 투영하고 HTML 리포트/MCP `recent_updates` 응답에서 바로 점검합니다.
...
- CI/CD workflows: `daily-collection.yml`, `deploy-pages.yml`, `pr-checks.yml`, `radar-crawler.yml`, `release.yml`.
- GitHub Pages visualization: `reports/index.html` (valid HTML); https://ai-frendly-datahub.github.io/HomeRadar/.
- Latest remote Pages check: HTTP 200, HTML.
- Local workspace audit: 98 Python files parsed, 0 syntax errors.
